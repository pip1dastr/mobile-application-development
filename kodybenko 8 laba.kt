...
fun main() {
    ...
    println("1. Сумма элементов списка: ${sumOfList(listOf(1, 2, 3, 4, 5))}")
    println("2. Разность max и min: ${differenceMaxMin(listOf(10, 2, 8, 4, 6))}")
    println("3. Объединенный список: ${mergeLists(listOf(1, 2, 3), listOf(4, 5, 6))}")
    println("4. Прибыльно? ${isProfitable(0.8, 100.0, 50.0)}")
    println("5. Прибыльно? ${isProfitable2(0.8, 100.0, 50.0)}")
    println("6. Сумма меньше 100? ${isSumLessThan100(30, 40)}")
    println("7. Делится на 100? ${isDivisibleBy100(200)}")
    println("8. Количество кадров: ${calculateFrames(2, 30)}")
    println("9. k^k равно n? ${isKPowerKEqualN(256, 4)}")
    println("10. Повторенная строка: ${repeatString("Hello", 3)}")
    println("11. Результат уравнения '1+1': ${evaluateExpression("1+1")}")
    println("12. Google: ${generateGoogle(3)}")
    greet()
    println("14. Сумма: ${sum(3, 5)}")
    println("15. Большее число: ${maxOfTwo(10, 20)}")
    println("16. Четное число? ${isEven(4)}")
    println("17. Факториал: ${factorial(5)}")
    println("18. Простое число? ${isPrime(7)}")
    println("19. Сумма массива: ${sumArray(intArrayOf(1, 2, 3, 4))}")
    println("20. Наибольшее число в массиве: ${maxInArray(intArrayOf(1, 5, 3, 9))}")
    println("21. Отсортированный массив: ${sortArray(intArrayOf(5, 3, 8, 1)).joinToString()}")
    println("22. Палиндром? ${isPalindrome("level")}")
    println("23. Количество символов: ${countCharacters("Hello")}")
    println("24. В верхнем регистре: ${toUpperCase("hello")}")
    println("25. Объединение строк: ${concatenateStrings("Hello, ", "world!")}")
    println("26. Последний элемент: ${lastElement(intArrayOf(1, 2, 3, 4, 5))}")
    println("27. Элемент присутствует? ${containsElement(intArrayOf(1, 2, 3, 4), 3)}")
    println("28. Массив от 1 до N: ${createArrayFrom1ToN(5).joinToString()}")
    println("29. Минимум: ${minMax(intArrayOf(3, 5, 1, 8, 2)).
    first}, Максимум: ${minMax(intArrayOf(3, 5, 1, 8, 2)).second}")
    println("30. Сумма от 1 до N: ${sumFrom1ToN(5)}") // Вывод: 15
    println("31. Температура в Фаренгейтах: ${celsiusToFahrenheit(25.0)}")
    println("32. Обратный порядок строки: ${reverseString("Hello")}")
    println("33. Элемент по индексу: ${findElementByIndex(intArrayOf(10, 20, 30), 1)}")
    println("34. Строка без пробелов: ${removeSpaces("Hello World")}")
    println("35. Сумма первых N натуральных чисел: ${sumFirstNNaturalNumbers(5)}")
    println("36. Подстрока присутствует? ${containsSubstring("Hello world", "world")}")
    println("37. Таблица умножения для 5:")
    printMultiplicationTable(5)
    println("38. Длина строки: ${stringLength("Hello")}")
    println("39. Перевернутый массив: ${reverseArray(intArrayOf(1, 2, 3, 4)).joinToString()}")
    println("40. Скопированный массив: ${copyArray(intArrayOf(1, 2, 3)).joinToString()}")
    println("41. Количество гласных в строке: ${countVowels("Hello World")}")
    println("42. Индекс первого вхождения элемента: ${indexOfFirstOccurrence(intArrayOf(1, 2, 3, 2), 2)}")
}